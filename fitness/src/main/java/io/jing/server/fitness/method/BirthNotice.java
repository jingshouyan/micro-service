...
@Slf4j
public class BirthNotice implements Method<NoticeInfo> {

    private static final DateTimeFormatter df = DateTimeFormatter.ofPattern("MMdd");
    ...
    public Object action(NoticeInfo noticeInfo) {
        ...
        List<Compare> compares = CompareUtil.newInstance()
                .field("birthday").gte(start).lte(end)
                .compares();
        Page<CustomBean> page = new Page<>();
        page.addOrderBy("birthday",false);
